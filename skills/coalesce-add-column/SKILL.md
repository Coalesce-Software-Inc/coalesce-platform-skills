...
- Use ONLY annotations the node accepts: the reserved column set (`@description("text")`, `@notNull`, `@defaultValue("...")`), `@isBusinessKey` (MERGE/SCD key; affects DDL), `@isChangeTracking` (change detection; affects DML), `@id("col-id")` (lineage; metadata-only), and whatever the node type DECLARES in its `annotations:` block (read its `definition.yml`; e.g. `@not_null`, `@uniqueness`, `@accepted_values("'A'")` repeated once per value). Do NOT invent annotations (e.g. `@isSurrogateKey`, `@pii`) — an undeclared one is accepted and silently does nothing. Never add or change the top-level `@id(...)`.
