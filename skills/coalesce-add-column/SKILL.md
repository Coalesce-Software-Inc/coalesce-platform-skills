...
description: Add a column to an existing Coalesce SQL (.sql, formerly V2) transformation node, placing any column annotation after the alias and before the comma, then verifying with coa.
---
...
- This skill targets SQL `.sql` nodes (formerly V2). If the file is a YAML `.yml` node (formerly V1), the edit is still in scope. Load `coalesce-v1-yaml-nodes` for the id graph, then append a column object to `operation.metadata.columns` with `name`, `dataType`, `nullable`, a `columnReference` of `{stepCounter: <this node's id>, columnCounter: <fresh UUID v4>}`, and `sourceColumnReferences` whose `columnReferences[]` names the upstream node's `id` plus that upstream column's `columnCounter` — for an upstream YAML node read it out of that file; for an upstream SQL node it is the column NAME as the warehouse sees it (uppercased unless the alias is quoted). For a computed column use `columnReferences: []` and put the expression in `transform`. Never reuse an existing `columnCounter` and never alter existing ids. Then run the step 4 verification and confirm the new column appears in the rendered DDL.
- For a SQL `.sql` node, confirm the node's `@nodeType(...)` resolves to a SQL node type (`fileVersion: 2`). If it does not, the new column will be SILENTLY DROPPED (`columns: []` → broken DDL/DML). The node's type already exists and is in use, so upgrading it is a shared-config change — STOP and ASK before bumping its `fileVersion` (see coalesce-workspace-config), rather than proceeding.
...
- Use ONLY annotations the node accepts: the reserved column set (`@description("text")`, `@notNull`, `@defaultValue("...")`), `@isBusinessKey` (MERGE/SCD key; affects DDL), `@isChangeTracking` (change detection; affects DML), and whatever the node type DECLARES in its `annotations:` block (read its `definition.yml`; e.g. `@not_null`, `@uniqueness`, `@accepted_values("'A'")` repeated once per value). Do NOT invent annotations (e.g. `@isSurrogateKey`, `@pii`) — an undeclared one is accepted and silently does nothing. NEVER write `@id` on a column: the new column's alias IS its column ID (uppercased unless the alias is quoted), and a column `@id` silently re-keys it so downstream references break (see sql-format reference, "Column identity"). Never add or change the top-level `@id(...)`.

## 4. Verify with coa (required)
1. `coa validate -d <dir> --include "{ <NODE_NAME> }"` — schema + graph scanners, scoped to this node. Scoping matters: the bundled example-repository currently fails `coa validate` with pre-existing legacy errors (environments, jobs, locations, nodeTypes, missing data.yml) that are UNRELATED to your column — disregard that noise and focus on diagnostics for the node you edited. (Schema validation always runs on all files; `--include` only scopes the graph scanners.) Note that on a SQL `.sql` node, columns and types are inferred from the SELECT, so the per-node column-reference scanner may not fire the way it does for YAML mapped columns — treat a clean validate as necessary but not sufficient, and confirm the column with the dry-run below. Alias the new expression; if its type renders as `UNKNOWN`, add an explicit cast.
